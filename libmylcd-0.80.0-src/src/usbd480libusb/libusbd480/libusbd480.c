...
static int setConfigDefaults (TUSBD480 *di)
{
	...
	libusbd480_SetConfigValue(di, CFG_TOUCH_DEBOUNCE_VALUE, 40);
	libusbd480_SetConfigValue(di, CFG_TOUCH_SKIP_SAMPLES, 100);
...
#if 0
	if (!pos->pressure) return -2;
	filter0(di, &di->touch, pos);
	return 1;
#endif
