...
int win_size_x = WIN_SIZE_X;
int win_size_y = WIN_SIZE_Y;
...
{
  if (width != height) {
    glutReshapeWindow(width > height ? height : width, width > height ? height : width);
    return;
  }
...
void touchCB (TTOUCHCOORD *pos, int flags, void *ptr)
{
	...
	mouse(GLUT_LEFT_BUTTON, GLUT_DOWN, pos->x, pos->y);
}
...
int main (int argc, char **argv)
{
  ...
  glutMainLoop();
  mylcdgl_setoption(lOPT_USBD480_TOUCHCB, 0);
  mylcdgl_shutdown();
  return 0;
}
