...
static void display_wheel (float w)
{
    int ww = w;
    glPopMatrix();
    glPushMatrix();
...
static void display_cart (float w)
{
    display_wheel(w);
    geel_texture();
...
static void Animate (void)
{
    ...
    angle2 = framect*4*PI/503;
    angle3 = framect*6*PI/503;
    framect++;
}
...
{
	const int myglflags = MYGL_CREATECONTEXT /*| MYGL_MESSAGEQUEUE*/ /* | MYGL_SHOWWINDOW*/;
    extern void calculate_rc (void);

    calculate_rc();
	mylcdgl_update(myglflags, 0, 0);
	mylcdgl_SetWindowTitle("Roller coaster");
