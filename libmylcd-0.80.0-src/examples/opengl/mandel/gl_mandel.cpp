...
#define C_PRESET_PATH "colourpresets"
...
static void  SaveImageAsPNG(wchar_t *path);
...
static void Initialize(void)
{
	...
	glutAddMenuEntry("Save Image as *.PNG File...", 1);
	glutAddMenuEntry("Save Image as *.TGA File...", 2);
	glutAddMenuEntry("Save Image as *.BMP File...", 3);
	glutAddMenuEntry("Save Rendering State...", 4);
	glutAddMenuEntry("Load Rendering State...", 5);
	glutAddMenuEntry("Set Min/Max Coords via Console...", 6);
	glutAddMenuEntry("Reload Color Presets", 7);
...
static void  SaveImageAsPNG (wchar_t *path)
{
	TFRAME *frame = lNewFrame(hw, _width, _height, LFRM_BPP_24);
	if (frame){
		unsigned char *data = (unsigned char *)malloc(_width *_height * 4);
		if (data){
			getGLFrame(data, frame, _width, _height);
			lSaveImage(frame, path, IMG_PNG, 0, 0);
			free(data);
		}
		lDeleteFrame(frame);
	}
...
static void MenuCallback_0(int id)
{
	...
	if (id == 1){
		snwprintf(wpath, sizeof(wpath), L"m%i.png", imgIndex++);
		SaveImageAsPNG(wpath);
		...
		snprintf(path, sizeof(path), "m%i.tga", imgIndex++);
		SaveImageAsTGA(path);
		...
		snprintf(path, sizeof(path), "m%i.bmp", imgIndex++);
		SaveImageAsBMP(path);
...
static pix_t Iterate(double cx, double cy, int &iter)
{
	double x = cx, y = cy, dx = 0.0, dy = 0.0, s = 0.0;
...
static void Redraw (void)
{
	...
	glFlush();
	//glutSwapBuffers();
	mylcdgl_update(MYGL_DRAWCROSSHAIR, _x, _y);
}
...
static void GLUT__Keyboard(unsigned char key, int x, int y)
{
	...
	if(key == 'p'){
		...
		DrawStatus(3);  // update rendering state
		BOX_UPDATE = 1; // this forces a status update
	}else if(key == 8 && ZOOM_COUNT > 1){ // BACKSPACE key restores prev. zoom state
		ZOOM_COUNT--;
		...
		Clear();
		GLUT__Reshape(_width, _height);
	}else if((key == 'j' || key == 'h') && !JULIA_MODE)	{
...
{
	//printf("touchCB: x:%i y:%i %i\n", pos->x, pos->y, state);
	_x = pos->x;
	_y = pos->y;
	GLUT__Motion(pos->x, pos->y);
	if (state)
		GLUT__Mouse(GLUT_LEFT_BUTTON, GLUT_UP, pos->x, pos->y);
	else
		GLUT__Mouse(GLUT_LEFT_BUTTON, GLUT_DOWN, pos->x, pos->y);
	state ^= 1;
}
...
int main (int argc, char *argv[])
{
	...
	fprintf(stdout, "Bernie Freidin, 1999-2008\n");
	fprintf(stdout, "Michael McElligott, 2008\n\n");
	fprintf(stdout, "(console is used for input)\n\n");
	...
	glutSetWindowTitle("OpenGL Mandel: Fractal Explorer");
	...
	mylcdgl_update(0, 0, 0);
	mylcdgl_setoption(lOPT_USBD480_TOUCHCB, (intptr_t*)touchCB);
	Initialize();
	glutMainLoop();
	mylcdgl_setoption(lOPT_USBD480_TOUCHCB, NULL);
	return 0;
}
